        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() => {
              ...
              // Add demographic insights section (if available)
              if (data.length > 0) {
                // Add a new page for demographic insights
                doc.addPage();
                
                // Add demographic insights header
                doc.setFontSize(16);
                doc.text('Demographic Insights', 15, 20);
                
                // Get key demographic data - look for repurchase intent question
                const demographicData = data;
                const repurchaseKeywords = ['purchase again', 'buy again', 'repurchase', 'reorder'];
                let repurchaseQuestion = null;
                let repurchaseQuestionText = '';
                
                // Find repurchase question
                for (let i = 1; i <= 10; i++) {
                  const num = i.toString().padStart(2, '0');
                  const questionKey = `question_${num}`;
                  const questionData = demographicData.find(item => item[questionKey]);
                  
                  if (questionData && questionData[questionKey]) {
                    const questionText = questionData[questionKey];
                    if (repurchaseKeywords.some(keyword => questionText.toLowerCase().includes(keyword))) {
                      repurchaseQuestion = num;
                      repurchaseQuestionText = questionText;
                      break;
                    }
                  }
                }
                ...
                if (repurchaseQuestion) {
                  // Add question text
                  doc.setFontSize(12);
                  doc.text(`Key Question: ${repurchaseQuestionText}`, 15, 30);
                  
                  // Analyze responses
                  const propositionKey = `proposition_${repurchaseQuestion}`;
                  const filteredData = demographicData.filter(item => 
                    item[propositionKey] && item[propositionKey].trim() !== '' && item.age_group
                  );
                  
                  // Get unique responses
                  const uniqueResponses = _.uniq(filteredData.map(item => item[propositionKey])).filter(Boolean);
                  
                  // Group by response
                  const responseGroups = _.groupBy(filteredData, propositionKey);
                  
                  // Create response summary table
                  const responseTableData = uniqueResponses.map(response => {
                    const count = responseGroups[response]?.length || 0;
                    return [
                      response,
                      count,
                  ...
                  // Add age group analysis for top response (if available)
                  if (responseTableData.length > 0) {
                    const topResponse = responseTableData[0][0];
                    const topResponseData = responseGroups[topResponse] || [];
                    
                    if (topResponseData.length > 0) {
                      // Group by age
                      const ageGroups = _.groupBy(topResponseData, 'age_group');
                      
                      // Prepare age distribution table
                      const ageTableData = Object.entries(ageGroups).map(([ageGroup, items]) => {
                        return [
                          ageGroup,
                          items.length,
                          `${((items.length / topResponseData.length) * 100).toFixed(1)}%`
                        ];
                      });
                      
                      // Sort by predefined age order
                      const sortedAgeTableData = ageTableData.sort((a, b) => {
                      ...
                      doc.setFontSize(12);
                      doc.text(`Age Distribution for "${topResponse}":`, 15, tableTop);
                      
                      doc.autoTable({
                        startY: tableTop + 5,
                        head: [['Age Group', 'Count', 'Percentage']],
                        body: sortedAgeTableData,
                        theme: 'grid',
                        headStyles: { fillColor: [255, 0, 102] },
                        styles: { fontSize: 10 },
                        margin: { left: 15 }
                  ...
                  doc.setFontSize(12);
                  doc.text('No repurchase intent question found in the data.', 15, 30);
                }
              }
              
              // Save the PDF with client name
              const fileName = clientName 
                ? `${clientName.toLowerCase().replace(/\s+/g, '-')}-analysis-report.pdf`
